...
const dashboardsSlice = createSlice({
  ...
  reducers: {
    ...
    reorderCards(state, action) {
      ...
      const sourceColumn = state.currentDashboard.columns.find(
        (column) => column._id === sourceDroppableId
      );
      const destinationColumn = state.currentDashboard.columns.find(
        (column) => column._id === destinationDroppableId
      );
      ...
      state.currentDashboard.columns = state.currentDashboard.columns.map(
        (column) => {
          if (column._id === sourceDroppableId) {
            return {
              ...column,
              cards: sourceCards,
            };
          }
          if (column._id === destinationDroppableId) {
            return {
              ...column,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(changeColumn.fulfilled, (state, action) => {
        ...
        const currentColumn = state.currentDashboard.columns.findIndex(
        ...
        state.currentDashboard.columns[currentColumn].cards.splice(
          indexCard,
          1
        );
...
export const { selectPriority, reorderCards } = dashboardsSlice.actions;
